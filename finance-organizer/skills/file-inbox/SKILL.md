...
3. **Route** to a set using `config.accounts` + content (see conventions). Business spend on a personal account is still business → book to the business set and log it in `config.overlap.personal_card_expense_doc`. **If unsure of the set or business-vs-personal, ask** — then offer to remember the answer (learn protocol).
